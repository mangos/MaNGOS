...
void Aura::_AddAura()
{
    ...
    if (IsNeedVisibleSlot(caster))
    {
        ...
        if (!secondaura)
        {
            ...
            SpellClassOptionsEntry const *classOpt = m_spellProto->GetSpellClassOptions();
            if (classOpt && classOpt->SpellFamilyName == SPELLFAMILY_WARLOCK &&
                // Immolate
                ((classOpt->SpellFamilyFlags & UI64LIT(0x0000000000000004)) ||
                // Shadowflame
                (classOpt->SpellFamilyFlags2 & 0x00000002)))
                m_target->ModifyAuraState(AURA_STATE_CONFLAGRATE, true);

            // Faerie Fire (druid versions)
            if (classOpt && classOpt->SpellFamilyName == SPELLFAMILY_DRUID && (classOpt->SpellFamilyFlags & UI64LIT(0x0000000000000400)))
                m_target->ModifyAuraState(AURA_STATE_FAERIE_FIRE, true);

            // Victorious
            if (classOpt && classOpt->SpellFamilyName == SPELLFAMILY_WARRIOR && (classOpt->SpellFamilyFlags & UI64LIT(0x0004000000000000)))
                m_target->ModifyAuraState(AURA_STATE_WARRIOR_VICTORY_RUSH, true);

            // Swiftmend state on Regrowth & Rejuvenation
            if (classOpt && classOpt->SpellFamilyName == SPELLFAMILY_DRUID && (classOpt->SpellFamilyFlags & UI64LIT(0x50)))
                m_target->ModifyAuraState(AURA_STATE_SWIFTMEND, true);

            // Deadly poison aura state
            if(classOpt && classOpt->SpellFamilyName == SPELLFAMILY_ROGUE && (classOpt->SpellFamilyFlags & UI64LIT(0x10000)))
...
bool Aura::_RemoveAura()
{
    ...
    if (m_removeMode != AURA_REMOVE_BY_DELETE)
    {
        ...
        SpellClassOptionsEntry const* classOpt = m_spellProto->GetSpellClassOptions();
        uint64 removeFamilyFlag = classOpt ? classOpt->SpellFamilyFlags : 0;
        uint32 removeFamilyFlag2 = classOpt ? classOpt->SpellFamilyFlags2 : 0;
        if(classOpt)
        {
            switch(classOpt->SpellFamilyName)
            {
                ...
                case SPELLFAMILY_WARLOCK:
                    // Conflagrate aura state on Immolate and Shadowflame,
                    if ((classOpt->SpellFamilyFlags & UI64LIT(0x0000000000000004)) ||
                        (classOpt->SpellFamilyFlags2 & 0x00000002))
                    {
                ...
                case SPELLFAMILY_DRUID:
                    if(classOpt->SpellFamilyFlags & UI64LIT(0x0000000000000400))
                        removeState = AURA_STATE_FAERIE_FIRE;   // Faerie Fire (druid versions)
                    else if(classOpt->SpellFamilyFlags & UI64LIT(0x50))
                    {
                ...
                case SPELLFAMILY_WARRIOR:
                    if(classOpt->SpellFamilyFlags & UI64LIT(0x0004000000000000))
                        removeState = AURA_STATE_WARRIOR_VICTORY_RUSH; // Victorious
                    break;
                case SPELLFAMILY_ROGUE:
                    if(classOpt->SpellFamilyFlags & UI64LIT(0x10000))
                        removeState = AURA_STATE_DEADLY_POISON; // Deadly poison aura state
                    break;
                case SPELLFAMILY_HUNTER:
                    if(classOpt->SpellFamilyFlags & UI64LIT(0x1000000000000000))
        ...
        if (removeState)
        {
            ...
            for(Unit::AuraMap::iterator i = Auras.begin(); i != Auras.end(); ++i)
            {
                ...
                SpellClassOptionsEntry const* aurClassOpt = auraSpellInfo->GetSpellClassOptions();
                if(aurClassOpt && aurClassOpt->SpellFamilyName == m_spellProto->GetSpellFamilyName() &&
                    (aurClassOpt->SpellFamilyFlags & removeFamilyFlag || aurClassOpt->SpellFamilyFlags2 & removeFamilyFlag2))
                {
                    found = true;
                    break;
                }
...
{
    uint32 stAmount = m_spellProto->GetStackAmount();
    // Can`t mod
    if (!stAmount)
        return true;
    ...
    if (stackAmount > (int32)stAmount)
        stackAmount = stAmount;
...
void Aura::HandleAddModifier(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        m_spellmod = new SpellModifier(
            ...
            GetSpellProto()->GetStackAmount() > 1 ? 0 : m_procCharges);
    }
...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
    ...
    else
    {
        ...
        if (m_removeMode == AURA_REMOVE_BY_DEATH)
        {
            ...
            if (classOptions && classOptions->SpellFamilyName == SPELLFAMILY_MAGE && (classOptions->SpellFamilyFlags & UI64LIT(0x0000000000000800)))
            {
                if (Unit* caster = GetCaster())
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            for (PlayerSpellMap::const_iterator itr = sp_list.begin(); itr != sp_list.end(); ++itr)
            {
                ...
                if (!spellInfo || !(spellInfo->Attributes & (SPELL_ATTR_PASSIVE | (1<<7))))
                    continue;
                if (spellInfo->GetStances() & (1<<(form-1)))
