...
void Aura::HandleAddModifier(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        if (spellProto->GetSpellFamilyName() == SPELLFAMILY_WARLOCK && spellProto->GetSpellIconId() == 3169)
        {
    ...
    switch (form)
    {
        ...
        case FORM_MOONKIN:
        {
            ...
            for (Unit::AuraList::const_iterator iter = slowingAuras.begin(); iter != slowingAuras.end();)
            {
                ...
                SpellMiscEntry const* spellMisc = aurSpellInfo->GetSpellMiscs();
                if (!spellMisc)
                    continue;
                ...
                case FORM_BERSERKERSTANCE:
                {
                    ...
                    if (target->GetTypeId() == TYPEID_PLAYER)
                    {
                        ...
                        for (PlayerSpellMap::const_iterator itr = sp_list.begin(); itr != sp_list.end(); ++itr)
                        {
                            ...
                            if (spellInfo->GetSpellFamilyName() == SPELLFAMILY_WARRIOR && spellInfo->GetSpellIconId() == 139)
...
void Aura::PeriodicTick()
{
    ...
    switch (m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
            ...
            if (classOptions && classOptions->SpellFamilyName==SPELLFAMILY_WARLOCK && (classOptions->SpellFamilyFlags & UI64LIT(0x0000000000000400)) && spellProto->GetSpellIconId() == 544)
            {
        ...
        case SPELL_AURA_OBS_MOD_HEALTH:
        {
            ...
            else
            {
                ...
                if (classOptions && classOptions->SpellFamilyName == SPELLFAMILY_DRUID && spellProto->GetSpellIconId() == 2864)
                {
...
//                pCaster->ProcDamageAndSpell(target, procAttacker, procVictim, PROC_EX_NORMAL_HIT, pdamage, BASE_ATTACK, spellProto);
        ...
        case SPELL_AURA_POWER_BURN_MANA:
        {
            ...
            SpellNonMeleeDamage damageInfo(pCaster, target, spellProto->Id, SpellSchoolMask(spellMisc->SchoolMask));
        ...
        case SPELLFAMILY_ROGUE:
        {
            switch (spell->Id)
            {
                ...
                case 51690:
                {
                    ...
                    {
                        // eff_radius ==0
                        float radius = GetSpellMaxRange(sSpellRangeStore.LookupEntry(spellMisc->RangeIndex));
    ...
    m_permanent(false), m_isRemovedOnShapeLost(true), m_deleted(false), m_in_use(0)
{
    ...
    SpellMiscEntry const* spellMisc = spellproto->GetSpellMiscs();
    if (m_maxDuration == -1 || (m_isPassive && (spellMisc && spellMisc->DurationIndex == 0)))
        m_permanent = true;
...
void SpellAuraHolder::HandleSpellSpecificBoosts(bool apply)
{
    ...
    switch(m_spellProto->GetSpellFamilyName())
    {
            ...
            if (classOptions && classOptions->SpellFamilyFlags & UI64LIT(0x0000040000000000))
            {
                if (!apply)
                {
                    ...
                    for (Unit::AuraList::const_iterator itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
                    {
                        ...
                        // Improved Fear
                        if (dummyEntry->GetSpellFamilyName() == SPELLFAMILY_WARLOCK && dummyEntry->GetSpellIconId() == 98)
                        {
                            cast_at_remove = true;
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            if ((m_spellProto->GetSpellIconId() == 234 && m_spellProto->GetSpellVisual(0)) || m_spellProto->GetSpellIconId() == 2213)
            {
                if (!apply && m_removeMode == AURA_REMOVE_BY_DISPEL)
                {
                    ...
                    for (Unit::AuraList::const_iterator itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
                    {
                        ...
                        if ((*itr)->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_PRIEST
                            && (*itr)->GetSpellProto()->GetSpellIconId() == 178)
                        {
                            // custom cast code
        ...
        case SPELLFAMILY_PALADIN:
        {
            if (m_spellProto->Id == 31884)                  // Avenging Wrath
            {
                ...
                else
                {
                    ...
                    for (Unit::AuraList::const_iterator itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
                    {
                        if ((*itr)->GetSpellProto()->GetSpellIconId() == 3029)
                        {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
                    ...
                    if (GetId() == 48263 || GetId() == 48266) // Frost Presence or Blood Presence
                    {
                        ...
                        if (apply)
                        {
                            ...
                            for (Unit::AuraList::const_iterator itr = unholyAuras.begin(); itr != unholyAuras.end(); ++itr)
                            {
                                // skip same icon
                                if ((*itr)->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_DEATHKNIGHT &&
                                    (*itr)->GetSpellProto()->GetSpellIconId() == 2633)
                                {
                    ...
                    if (GetId() == 48265 || GetId() == 48266)   // Unholy Presence or Blood Presence
                    {
                        ...
                        if (apply)
                        {
                            ...
                            for (Unit::AuraList::const_iterator itr = frostAuras.begin(); itr != frostAuras.end(); ++itr)
                            {
                                // skip same icon
                                if ((*itr)->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_DEATHKNIGHT &&
                                    (*itr)->GetSpellProto()->GetSpellIconId() == 2632)
                                {
            ...
            // Improved Frost Presence
            if (GetSpellProto()->GetSpellIconId() == 2632 && m_isPassive)
            {
            ...
            // Improved Unholy Presence
            if (GetSpellProto()->GetSpellIconId() == 2633 && m_isPassive)
            {
...
void SpellAuraHolder::Update(uint32 diff)
{
    ...
    if (IsChanneledSpell(m_spellProto) && GetCasterGuid() != m_target->GetObjectGuid())
    {
        ...
        if (caster->GetChannelObjectGuid() == m_target->GetObjectGuid())
        {
            ...
            // Get spell range
            float max_range = GetSpellMaxRange(sSpellRangeStore.LookupEntry(spellMisc->RangeIndex));
...
void SpellAuraHolder::SetAuraMaxDuration(int32 duration)
{
    ...
    if (duration > 0)
    {
        ...
        if (!(IsPassive() && (spellMisc && spellMisc->DurationIndex == 0)))
            SetPermanent(false);
