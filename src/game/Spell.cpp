...
void Spell::AddUnitTarget(Unit* pVictim, SpellEffectIndex effIndex)
{
    ...
    if (spellMisc && spellMisc->Speed > 0.0f && affectiveObject && pVictim != affectiveObject)
    {
        // calculate spell incoming interval
        float dist = affectiveObject->GetDistance(pVictim->GetPositionX(), pVictim->GetPositionY(), pVictim->GetPositionZ());
        if (dist < 5.0f)
            dist = 5.0f;
        target.timeDelay = (uint64) floor(dist / spellMisc->Speed * 1000.0f);
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    if (spellMisc && spellMisc->Speed > 0)
    {
...
void Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask)
{
    ...
    if (realCaster && realCaster != unit)
    {
        ...
        if (spellMisc && spellMisc->Speed > 0.0f &&
        ...
        if (!realCaster->IsFriendlyTo(unit))
        {
            ...
            if (spellMisc && spellMisc->Speed > 0.0f && unit == m_targets.getUnitTarget() &&
        ...
        else
        {
            ...
            if (spellMisc && spellMisc->Speed > 0.0f && !IsPositiveSpell(m_spellInfo->Id))
            {
                realCaster->SendSpellMiss(unit, m_spellInfo->Id, SPELL_MISS_EVADE);
                ResetEffectDamageAndHeal();
                return;
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    if (spellEffect && spellEffect->EffectRadiusIndex)
        ...
        radius = GetSpellMaxRange(sSpellRangeStore.LookupEntry(spellMisc->RangeIndex));
    ...
    switch (targetMode)
    {
        ...
        case TARGET_ALL_FRIENDLY_UNITS_IN_AREA:
            ...
            // Circle of Healing
            else if (m_spellInfo->GetSpellFamilyName() == SPELLFAMILY_PRIEST && m_spellInfo->GetSpellVisual(0) == 8253)
            {
        ...
        {
            bool inFront = m_spellInfo->GetSpellVisual(0) != 3879;
        ...
        case TARGET_DIRECTLY_FORWARD:
        {
            if (!(m_targets.m_targetMask & TARGET_FLAG_DEST_LOCATION))
            {
                SpellRangeEntry const* rEntry = sSpellRangeStore.LookupEntry(spellMisc->RangeIndex);
        ...
        case TARGET_EFFECT_SELECT:
        {
            ...
            switch(spellEffect->Effect)
            {
                ...
                case SPELL_EFFECT_APPLY_AREA_AURA_PARTY:
                    // AreaAura
                    if ((spellMisc->Attributes == (SPELL_ATTR_NOT_SHAPESHIFT | SPELL_ATTR_UNK18 | SPELL_ATTR_CASTABLE_WHILE_MOUNTED | SPELL_ATTR_CASTABLE_WHILE_SITTING)) || (spellMisc->Attributes == SPELL_ATTR_NOT_SHAPESHIFT))
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if(Unit *target = m_targets.getUnitTarget())
    {
        ...
        if (spellMisc && spellMisc->AttributesEx2 == SPELL_ATTR_EX2_UNK20 && m_spellInfo->HasAttribute(SPELL_ATTR_EX_UNK9) && target->HasInArc(M_PI_F, m_caster))
        {
        ...
        if ((spellMisc && spellMisc->Attributes == (SPELL_ATTR_UNK4 | SPELL_ATTR_NOT_SHAPESHIFT | SPELL_ATTR_UNK18 | SPELL_ATTR_STOP_ATTACK_TARGET)) && !target->HasInArc(M_PI_F, m_caster))
        {
            SendInterrupted(2);
    ...
    if (m_UniqueTargetInfo.empty())                         // skip second CheckCast apply (for delayed spells for example)
    {
        for (int j = 0; j < MAX_EFFECT_INDEX; ++j)
        {
               ...
               spellEffect->EffectImplicitTargetA == TARGET_FOCUS_OR_SCRIPTED_GAMEOBJECT)
            {
                ...
                SpellRangeEntry const* srange = sSpellRangeStore.LookupEntry(spellMisc && spellMisc->RangeIndex);
